...
export default async function signUpGameInGroup(
  ...
  if (/^\#.+\+1$/g.test(message)) {
    ...
    info(`${unknowUser.displayName} order successful! by ${user.displayName}`);
    ...
    info(`${name}'s order is cancelled by ${user.displayName}`);

    //
    // handle +1
  } else if (/^\+1$/g.test(message)) {
    game = await addUserToGame(user, lastestGame);
    info(`[+1] ${user.displayName} order successful!`);

    //
    // handle -1
  } else if (/^\-1$/g.test(message)) {
    game = await removeUserFromGame(user, lastestGame);
    info(`[-1] ${user.displayName}'s order cancelled.`);
  } else {
    info("no match event!");
    return;
  }
